...
function Install-LocalMaven { param([scriptblock]$LogCallback) if(-not $LogCallback){$LogCallback={param($m)}}; $mavenRoot=Join-Path $env:APPDATA "Mall4jLauncher\maven"; $mvnExe=Join-Path $mavenRoot "bin\mvn.cmd"; if(Test-Path $mvnExe){& $LogCallback "Local Maven found: $mvnExe";return $mvnExe}; $version="3.9.9";$zipUrl="https://dlcdn.apache.org/maven/maven-3/$version/binaries/apache-maven-$version-bin.zip";$zipPath="$env:TEMP\apache-maven-$version-bin.zip";& $LogCallback "Downloading Maven $version from Apache (may take a while)...";try{[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12;$wc=New-Object System.Net.WebClient;& $LogCallback "  $zipUrl";$wc.DownloadFile($zipUrl,$zipPath);$wc.Dispose()}catch{& $LogCallback "Primary mirror failed, trying backup..." -Level "WARN";try{$wc=New-Object System.Net.WebClient;$wc.DownloadFile("https://archive.apache.org/dist/maven/maven-3/$version/binaries/apache-maven-$version-bin.zip",$zipPath);$wc.Dispose()}catch{& $LogCallback "Download failed: $_" -Level "ERROR";return $null}};& $LogCallback "Extracting Maven...";try{$tempExtract="$env:TEMP\maven-extract";Expand-Archive -Path $zipPath -DestinationPath $tempExtract -Force;$extracted=Join-Path $tempExtract "apache-maven-$version";if(-not (Test-Path $extracted)){$extracted=Get-ChildItem $tempExtract -Directory|Select-Object -First 1 -ExpandProperty FullName};if(-not (Test-Path $mavenRoot)){New-Item -ItemType Directory -Path $mavenRoot -Force|Out-Null};Move-Item -Path "$extracted\*" -Destination $mavenRoot -Force;Remove-Item -Path $tempExtract -Recurse -Force;Remove-Item $zipPath -Force -ErrorAction SilentlyContinue}catch{& $LogCallback "Extraction failed: $_" -Level "ERROR";return $null};if(Test-Path $mvnExe){& $LogCallback "Maven installed locally: $mvnExe" -Level "SUCCESS";return $mvnExe};$mvnExe2=Join-Path $mavenRoot "bin\mvn";if(Test-Path $mvnExe2){return $mvnExe2};& $LogCallback "Maven install failed: $mvnExe not found" -Level "ERROR";return $null }
function Invoke-MavenBuild { param([string]$ProjectRoot,[string]$Module="",[scriptblock]$LogCallback,[int]$TimeoutSeconds=300,[string]$MvnPath="mvn") if(-not $LogCallback){$LogCallback={param($m)}}; $result=@{Success=$false;Message="";Duration=0}; $mvnArgs=@("clean","package","-DskipTests"); if($Module){$mvnArgs=@("clean","package","-pl","yami-shop-$Module","-am","-DskipTests")}; $moduleLabel=if($Module){$Module}else{"all modules"}; & $LogCallback "Building backend ($moduleLabel)..."; & $LogCallback "  $MvnPath $($mvnArgs -join ' ')"; $startTime=Get-Date;$logFile="$env:TEMP\mall4j_maven_build.log"; try{$proc=Start-Process -FilePath $MvnPath -ArgumentList $mvnArgs -NoNewWindow -PassThru -WorkingDirectory $ProjectRoot -RedirectStandardOutput $logFile -RedirectStandardError "${logFile}.err";$proc.WaitForExit($TimeoutSeconds*1000);$duration=[int]((Get-Date)-$startTime).TotalSeconds;$result.Duration=$duration;if($proc.ExitCode -eq 0){$result.Success=$true;$result.Message="Build OK (${duration}s)";& $LogCallback "Build OK (${duration}s)"}else{& $LogCallback "Build FAILED";Get-Content $logFile -Tail 10 -ErrorAction SilentlyContinue|ForEach-Object{& $LogCallback "  $_"};$result.Message="Build failed"}}catch{$result.Message="Build error: $_";& $LogCallback "Build error: $_"}; return $result }
...
try {
    ...
    if ($jarsExist.AllExist) {
        ...
        # JAR 不存在，自动下载 Maven
        Write-LogUI -Message "Maven not found, downloading locally..." -Level "INFO"
        Set-StepUI -Id 3 -Status "running" -Text "Downloading Maven..."
        $mvnPath = Install-LocalMaven -LogCallback $logCb
        if (-not $mvnPath) {
            Write-LogUI -Message "Failed to download Maven. Check internet connection." -Level "ERROR"
        ...
        try {
            $mavenResult = Invoke-MavenBuild -ProjectRoot $ProjectRoot -LogCallback $logCb -TimeoutSeconds 300 -MvnPath $mvnPath
        } catch {
